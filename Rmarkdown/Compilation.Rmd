...
output$NetworkPlot <- renderPlotly({
      ...
      geom_point(data=uframe, aes(size=eoyForecast,text=sprintf("Reservoir: %s",User), frame=MoYear, ids=User), alpha=0.5) +
    ...
    gg <- ggplotly(gg) 
    style(gg, hoveron="points", hoverinfo = "none")
    
   })


fluidRow(
  shiny::column(4, offset=4, selectInput("scaleby","Scale nodes by:",c("End of Month Use", "End of Year Forecast", "Approved Annual Diversion", "Forecasted Annual Excess"))
  )
)
...
fluidRow(
  ...
  column(6, div(style = "height: 550px; background-color: #fcfcfc;")
  )
)
